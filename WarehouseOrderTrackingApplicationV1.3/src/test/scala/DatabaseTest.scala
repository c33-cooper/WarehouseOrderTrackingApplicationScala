...
class DatabaseTest extends TestBase {
  ...
  def findAllSQLTablesTest(){
    ...
    it should "add valid data to column 5 of the table" in
    {
      // Create an object and run test
      val database : Database = new Database
      database findAllSQLTables("""SELECT idorders, orderDateCreated,
           orderVerified, orderTotal, productQuantity, orderStatus 
           FROM orders""") findColumn("productQuantity") should be (5)
    }
    it should "add valid data to column 6 of the table" in
    {
      // Create an object and run test
      val database : Database = new Database
      database findAllSQLTables("""SELECT idorders, orderDateCreated,
           orderVerified, orderTotal, productQuantity, orderStatus 
           FROM orders""") findColumn("orderStatus") should be (6)
    }
  }
